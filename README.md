...
只丟 PDF、不選訂單總表時,程式改用**託運單上印的「備註」欄**分組 ——
那段文字就是出貨表寫進去的品項描述(商品名稱 + 規格設定 + 客製刻印選項)。
可以切換兩種細度:

| 模式 | 分法 | 76 頁的實測 |
|---|---|---|
| 依品項(預設) | 只看品項名,另外把「星座」和「金運/招福」併進 key(這兩個撿貨一定要分開),再套下面的別名規則 | 13 組 |
| 依品項+顏色 | 連顏色/規格一起分 | 33 組 |

一樣套用上面的「合併門檻」:沒超過門檻的品項併成一份「其他合併」,
門檻設 1 就每組都獨立出檔。

有兩組是靠商品編號合併的,純文字看不出來,所以 `PDF_ALIAS_RULES` 用關鍵字補上,
結果才會跟有訂單總表時一致:

| 規則 | 為什麼 |
|---|---|
| 含「雷雕」「刻印」→ 一律歸「雷雕客製刻印(不分商品/尺寸)」 | 撿完都送去同一個雷雕站,不分商品也不分尺寸 |
| 含「繽紛好運精油組」「精油禮盒」→ 歸「精油組(尺寸)」 | 禮盒版和一般版是同一個實體商品,只是上架名稱不同 |

賣場上架新的同義寫法時,補進那張表。

載入訂單總表之後會自動改回上面那條路(能用到商品編號、別名、尺寸那些規則,分得更準)。

> **為什麼要自己解碼**:黑貓這份 PDF 的內嵌字型沒給正確的 ToUnicode 對照,
> pdf.js 解出來的中文整個被平移(「禮」變成「䥖」),pdfplumber 之類的工具會自己補救,
> 瀏覽器端沒有。`zoned-pdf.js` 的做法是拿「每張單一定會印的標籤」(備註/訂單編號/收件人)
> 去反推平移量 —— 平移不會改變字與字的差值,對到兩個以上標籤才算數。
> 另外 pdf.js 給的是**繪製順序**不是閱讀順序(標籤會一次畫完、數值另外畫),
> 所以要先照座標把同一列的片段拼回去,標籤才會跟自己的值配在一起。
